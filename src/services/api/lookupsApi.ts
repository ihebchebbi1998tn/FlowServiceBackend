...
const articlesGroupsApi = {
  async getAll(): Promise<LookupListResponse> {
    try {
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({ message: 'Failed to fetch article groups' }));
        throw new Error(error.message || 'Failed to fetch article groups');
      }
  ...
  async getById(id: string): Promise<LookupItem> {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch article group');
    }
