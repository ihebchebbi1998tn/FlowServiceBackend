...
export const generateFollowUpQuestions = async (
  ...
  try {
    const systemPrompt = language === 'fr' 
      ? `Tu es un assistant qui génère des questions de suivi pertinentes pour FlowService (application de gestion de services terrain et CRM). 
...
- Être pertinentes pour FlowService
Réponds UNIQUEMENT avec un tableau JSON de 3 questions, sans explication.`
      : `You are an assistant that generates relevant follow-up questions for FlowService (a field service management and CRM application).
...
- Be relevant to FlowService features
