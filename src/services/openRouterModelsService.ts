...
/**
 * Returns usable (unmasked) API keys for making OpenRouter API calls.
 * Uses the local cache since the backend masks keys on GET.
 */
export function getUsableApiKeys(): string[] {
  return getLocalKeys()
    .sort((a, b) => a.priority - b.priority)
    .map(k => k.apiKey)
    .filter(k => k.startsWith('sk-')); // only valid keys
}
