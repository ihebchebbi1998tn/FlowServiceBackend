    ...
    financialCalculations: {
      ...
      overview: {
        ...
        fields: [
          ...
          { name: "totalAmount", type: "decimal", description: "Final total = (subtotal - discount) + tax + fiscalStamp" }
        ]
      },
...
│                                                              │
│   TAX = AFTER_DISCOUNT × taxRate  (if percentage)            │
│                                                              │
│   TOTAL = AFTER_DISCOUNT + TAX + FISCAL_STAMP                │
│                                                              │
│   Example:                                                   │
│   ─────────────────────────────────────                      │
│   Item 1: 2 × 500 = 1,000 TND                               │
│   Item 2: 1 × 1,500 = 1,500 TND                             │
│   ─────────────────────────────────────                      │
│   Subtotal:           2,500 TND                              │
│   Discount (fixed):   -  100 TND                             │
│   After Discount:     2,400 TND                              │
│   TVA (19%):          +  456 TND                             │
│   Fiscal Stamp:       +    1 TND                             │
│   ─────────────────────────────────────                      │
│   TOTAL:              2,857 TND                              │
│                                                              │
└─────────────────────────────────────────────────────────────┘`,
        frontendCode: `// Frontend: uses centralized calculateDocumentTotal()
...
// totals.afterDiscount = subtotal - discountAmount
// totals.taxAmount = afterDiscount × taxRate
// totals.total = afterDiscount + taxAmount + fiscalStamp`,
...
CREATE TABLE Offers (
  ...
  "total_amount" DECIMAL(15,2) GENERATED ALWAYS AS (
    "amount" + COALESCE("taxes", 0) - COALESCE("discount", 0)
        ...
        pdfCode: `// PDF Generation — uses centralized calculateEntityTotal()
import { calculateEntityTotal } from '@/lib/calculateTotal';

const FinancialSummary = ({ offer }) => {
  const totals = calculateEntityTotal(offer);
  // totals.subtotal → totals.afterDiscount → totals.taxAmount → totals.total
  return (
    <View>
      <Text>Subtotal: {formatCurrency(totals.subtotal)}</Text>
      <Text>Remise: -{formatCurrency(totals.discountAmount)}</Text>
      <Text>Net HT: {formatCurrency(totals.afterDiscount)}</Text>
      <Text>TVA: +{formatCurrency(totals.taxAmount)}</Text>
      <Text>Total TTC: {formatCurrency(totals.total)}</Text>
    </View>
  );
};`
      },
      ...
      features: [
        {
          ...
          details: [
            ...
            "Automatic total calculation: (subtotal - discount) + TVA + fiscal stamp",
