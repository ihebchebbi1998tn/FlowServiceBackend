...
export default function ContactsPage() {
  ...
  const contactImportConfig: ImportConfig<any> = {
    ...
    fields: [
      ...
      },
      {
        ...
        validate: (value: string) => {
          ...
          if (!['individual', 'company', 'partner'].includes(normalized)) {
            return t('bulkImport.validation.invalidType', 'Invalid type. Allowed: individual, company, partner');
          }
          return null;
        }
      },
      {
        ...
        validate: (value: string) => {
          ...
          if (!validStatuses.includes(value.toLowerCase().trim())) {
            return t('bulkImport.validation.invalidStatus', 'Invalid status. Allowed: active, inactive, lead, customer, partner');
          }
          return null;
        }
      },
    ...
    transformRow: (data) => ({
      ...
      type: (['individual', 'company', 'partner'].includes(data.type?.toLowerCase?.()?.trim?.())
        ? data.type.toLowerCase().trim()
        : 'individual'),
      status: (['active', 'inactive', 'lead', 'customer', 'partner'].includes(data.status?.toLowerCase?.()?.trim?.())
