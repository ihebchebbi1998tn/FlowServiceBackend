...
import { useCategories, useLocations } from '@/modules/articles/hooks/useArticles';
...
export function ArticleForm({ article, onSubmit, onCancel, isSubmitting }: ArticleFormProps) {
  ...
  const [formData, setFormData] = useState<CreateArticleRequest & { categoryId?: number; locationId?: number }>({
      ...
      {formData.type === 'material' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="subLocation">{t('add.sub_location')}</Label>
              <Input
                id="subLocation"
