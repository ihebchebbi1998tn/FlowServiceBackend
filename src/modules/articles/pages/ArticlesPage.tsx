    ...
    fields: [
      ...
      { 
        ...
        validate: (value: string) => {
          ...
          if (!['material', 'service'].includes(normalized) && 
              !normalized.includes('material') && !normalized.includes('service')) {
            return t('bulkImport.validation.invalidType', 'Invalid type. Allowed values: material, service');
          }
          return null;
        }
      },
      ...
      { 
        ...
        validate: (value: string) => {
          if (!value) return null;
          const validStatuses = ['available', 'active', 'low_stock', 'out_of_stock', 'discontinued', 'inactive'];
          if (!validStatuses.includes(value.toLowerCase().trim())) {
            return t('bulkImport.validation.invalidStatus', 'Invalid status. Allowed: available, active, low_stock, out_of_stock, discontinued, inactive');
          }
          return null;
        }
      },
      { 
        ...
        validate: (value: any) => {
          ...
          if (isNaN(num) || num < 0) {
            return t('bulkImport.validation.invalidStock', 'Stock must be a positive number');
          }
          return null;
        }
      },
      { 
        ...
        validate: (value: any) => {
          ...
          const num = Number(value);
          if (isNaN(num) || num < 0) {
            return t('bulkImport.validation.invalidMinStock', 'Min stock must be a positive number');
          }
          return null;
        }
      },
      { 
        ...
        validate: (value: any) => {
          ...
          const num = Number(value);
          if (isNaN(num) || num < 0) {
            return t('bulkImport.validation.invalidPrice', 'Price must be a positive number');
          }
          return null;
        }
      },
      ...
      { 
        ...
        validate: (value: any) => {
          ...
          const num = Number(value);
          if (isNaN(num) || num < 0) {
            return t('bulkImport.validation.invalidDuration', 'Duration must be a positive number');
          }
          return null;
        }
      },
      ...
      {pagination && pagination.pages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            {t('pagination.showing', { count: articles.length, total: pagination.total })}
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
