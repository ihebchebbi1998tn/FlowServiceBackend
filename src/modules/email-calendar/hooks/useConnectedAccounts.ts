...
function openOAuthPopup(authUrl: string, clientId: string, redirectUri: string, scopes: string[], provider: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    // Poll for the redirect with the code
      ...
      try {
        ...
        if (popupUrl.startsWith(redirectUri)) {
          ...
          if (code) {
            ...
            reject(new Error(urlParams.get('error') || 'No authorization code received'));
          }
        }
      } catch {
...
export function useConnectedAccounts() {
  ...
  const connectAccount = useCallback(async (provider: EmailCalendarProvider) => {
    ...
    try {
      ...
      // Use the centralized OAuth redirect URI (registered in Google Console)
      // to avoid redirect_uri_mismatch errors in preview/dev environments
      const { getOAuthCallbackUrl } = await import('@/config/api');
      const redirectUri = getOAuthCallbackUrl(provider as 'google' | 'microsoft');
