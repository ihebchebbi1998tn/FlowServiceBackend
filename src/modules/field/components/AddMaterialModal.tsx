          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                id="quantity"
                type="number"
                min={isDecimalUnit(selectedUnit) ? "0.01" : "1"}
                step={isDecimalUnit(selectedUnit) ? "0.01" : "1"}
                value={formData.quantity}
                onChange={(e) => setFormData({ ...formData, quantity: isDecimalUnit(selectedUnit) ? (parseFloat(e.target.value) || 0.01) : (parseInt(e.target.value) || 1) })}
                required
              />
            </div>
