...
export function AddTimeExpenseDialog({
  ...
  const handleSubmitTime = async () => {
    ...
    try {
      ...
      // Compute start/end from duration (end = now, start = now - duration)
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[520px] p-0 gap-0">
        <DialogHeader className="px-6 pt-6 pb-4">
          <DialogTitle className="text-lg">{t('time-expenses:add_entry.title')}</DialogTitle>
        </DialogHeader>

        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'time' | 'expense')} className="w-full">
          <div className="px-6">
          ...
          <ScrollArea className="max-h-[60vh]">
            {/* ─── TIME ENTRY TAB ─── */}
            <TabsContent value="time" className="px-6 pb-6 pt-4 space-y-4 mt-0">
              {/* Dispatch */}
              ...
              {/* Submit */}
              <Button onClick={handleSubmitTime} disabled={submitting} className="w-full">
                {submitting ? t('time-expenses:add_entry.submitting') : t('time-expenses:add_entry.submit_time')}
              </Button>
            </TabsContent>

            {/* ─── EXPENSE TAB ─── */}
            <TabsContent value="expense" className="px-6 pb-6 pt-4 space-y-4 mt-0">
              {/* Dispatch */}
              <div className="space-y-1.5">
                <Label className="text-sm">{t('time-expenses:add_entry.dispatch')}</Label>
              ...
              {/* Submit */}
              <Button onClick={handleSubmitExpense} disabled={submitting} className="w-full">
                {submitting ? t('time-expenses:add_entry.submitting') : t('time-expenses:add_entry.submit_expense')}
              </Button>
            </TabsContent>
          </ScrollArea>
        </Tabs>
