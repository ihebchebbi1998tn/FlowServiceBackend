...
export function OfferPDFPreviewModal({
    ...
    const fetchInstallations = async () => {
      ...
      await Promise.all(installationIds.map(async (id) => {
        try {
          ...
          data[id] = { name: inst.name, model: inst.model, serialNumber: inst.serialNumber, manufacturer: inst.manufacturer };
        } catch { /* skip */ }
      }));
  ...
  const pdfDocElement = useMemo(() => (
    <OfferPDFDocument offer={offer} formatCurrency={formatCurrency} settings={pdfSettings} translations={pdfTranslations} installationsData={installationsData} />
  ), [offer, formatCurrency, pdfSettings, pdfTranslations, installationsData]);
