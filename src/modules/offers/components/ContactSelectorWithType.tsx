...
export function ContactSelectorWithType({ onContactSelect, selectedContact, disabled = false }: ContactSelectorWithTypeProps) {
  ...
  const handleQuickAdd = async () => {
    ...
    // Validation - match AddContact page requirements
    if (isCompany) {
  ...
  if (showQuickAdd) {
    const canProceedToStep2 =
      quickAddData.type === 'individual'
        ? Boolean(quickAddData.firstName && quickAddData.lastName)
        : Boolean(quickAddData.company);
        ...
        {quickAddStep === 1 ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Matricule Fiscale for companies - next to phone */}
              {quickAddData.type === 'company' && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder={t('matriculeFiscalePlaceholder')}
                  />
                </div>
              )}
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Input
                  ...
                  placeholder={quickAddData.type === 'company' ? '123 Business Ave, City, Country' : '123 Main St, City, Country'}
                />
              </div>
            </div>

            {/* CIN and Matricule Fiscale for individuals - before Work Information */}
            {quickAddData.type === 'individual' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="quick-matriculeFiscale-ind">{t('matriculeFiscale')}</Label>
                  <Input
                    id="quick-matriculeFiscale-ind"
                    value={quickAddData.matriculeFiscale}
                    onChange={(e) => setQuickAddData(prev => ({ ...prev, matriculeFiscale: e.target.value }))}
                    placeholder={t('matriculeFiscalePlaceholder')}
                  />
            ...
            {quickAddData.type === 'individual' && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="quick-status">{t('contactStatus')}</Label>
                    <Select 
                      value={quickAddData.status} 
                      onValueChange={(value) => setQuickAddData(prev => ({ ...prev, status: value }))}
                    >
                      <SelectTrigger>
            ...
            {quickAddStep === 1 ? (
              ...
              <Button 
                ...
                disabled={
                  quickAddLoading ||
                  (quickAddData.type === 'individual'
                    ? (!quickAddData.firstName || !quickAddData.lastName)
                    : !quickAddData.company)
                }
                className="gap-2"
              >
                {quickAddLoading ? (
