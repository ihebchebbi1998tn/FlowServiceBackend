...
import { X, Send, Loader2, MessageSquare, ThumbsUp, ThumbsDown, Copy, Check, RefreshCw, Mic, MicOff, Volume2, VolumeX, AtSign, CheckCircle2, Pencil, Trash2, Plus, CalendarIcon, Sparkles, HelpCircle, Menu, Slash, Settings, ImagePlus } from 'lucide-react';
...
export function AiAssistantSidebar({ isOpen, onClose }: AiAssistantSidebarProps) {
  ...
  const handleSendMessage = async () => {
    ...
    // Check if this is a form creation request FIRST.
    // This prevents the generic assistant (with strict business-only boundaries) from answering/refusing
    // before we generate the form.
    const formCreationCheck = await detectFormCreationIntentSmart(trimmedInput);
    const isFormCreationIntent = formCreationCheck.hasIntent;

    // Check if this is a task creation request (skip if it's a form request)
    let isTaskCreationIntent = false;
    if (!isFormCreationIntent) {
      const taskCreationCheck = await detectTaskCreationIntentSmart(trimmedInput);
      isTaskCreationIntent = taskCreationCheck.hasIntent;
    }
    ...
    try {
      ...
      const response = isFormCreationIntent
        ? await streamMessageToAIWithSystemPrompt(
            getFormCreationPrompt(formCreationCheck.language, formCreationCheck.extractedContext),
        ...
        >
          {messages.length === 0 ? (
            <div className="flex flex-col px-4 sm:px-6 pt-8 pb-4">
              ...
              {getUsableApiKeys().length === 0 && (
                <div className="mb-4 flex items-start gap-2.5 p-3 rounded-lg border border-destructive/30 bg-destructive/5">
                  <Settings className="h-4 w-4 mt-0.5 shrink-0 text-destructive" />
                  <div className="text-xs leading-relaxed">
                    <p className="font-medium mb-0.5 text-destructive">{t('noApiKeysTitle')}</p>
